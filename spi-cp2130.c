...
#define CP2130_IRQ_POLL_INTERVAL     1 * 1000 * 1000	/* in us */
		...
		*del = '\0';
		...
		case 8:	/* inter-byte delay */
			...
			ret |= (chn.inter_byte_delay < 0 ||
				chn.inter_byte_delay > 0xffff) ? -EINVAL : 0;
			if (ret)
				goto out;
			break;
		case 9:	/* pre-deassert delay */
			...
			ret |= (chn.pre_deassert_delay < 0 ||
				chn.pre_deassert_delay > 0xffff) ? -EINVAL : 0;
			if (ret)
				goto out;
			break;
		case 10:	/* post-assert delay */
			...
			ret |= (chn.post_assert_delay < 0 ||
				chn.post_assert_delay > 0xffff) ? -EINVAL : 0;
			if (ret)
				goto out;
			break;
		case 11:	/* modalias */
					   ...
					   struct spi_message *mesg)
{
	...
	dev_dbg(&master->dev, "recv/xmit pipes: %u / %u", recv_pipe, xmit_pipe);
	...
	if (chn_id != dev->current_channel) {
		...
		ret = usb_control_msg(dev->udev, xmit_ctrl_pipe,
				      CP2130_BREQ_SET_GPIO_CS,
				      USB_TYPE_VENDOR | USB_RECIP_DEVICE |
				      USB_DIR_OUT, 0, 0, ctrl_urb, 2, 200);
		if (ret < 2)
			goto err;
	...
	list_for_each_entry(xfer, &mesg->transfers, transfer_list) {
		...
		*((u32 *) (urb + CP2130_BULK_OFFSET_LENGTH)) =
		...
		/* copy SPI data */
		memcpy(urb + CP2130_BULK_OFFSET_DATA, xfer->tx_buf, xfer->len);
...
static void cp2130_update_channel_config(struct work_struct *work)
{
	...
	dev_dbg(&dev->udev->dev, "control pipes: %u, %u",
		usb_sndctrlpipe(dev->udev, 0), usb_rcvctrlpipe(dev->udev, 0));
	...
	for (i = 0; i < CP2130_NUM_GPIOS; i++) {
		...
		urb[1] = (chn->clock_freq << 0) |
		    (chn->cs_pin_mode << 3) |
		    (chn->polarity << 4) | (chn->clock_phase << 5);

		ret = usb_control_msg(dev->udev, xmit_pipe,
				      CP2130_BREQ_SET_SPI_WORD,
				      USB_TYPE_VENDOR | USB_RECIP_DEVICE |
				      USB_DIR_OUT, 0, 0, urb, 2, 200);
		if (ret < 2)
			goto error_unlock;
...
static void cp2130_read_channel_config(struct work_struct *work)
{
	...
	ret = usb_control_msg(dev->udev, recv_pipe,
...
static void cp2130_update_otprom(struct work_struct *work)
{
	...
	ret = usb_control_msg(dev->udev, xmit_pipe,
			      ...
			      CP2130_BREQ_MEMORY_KEY, 0, urb, 0x14, 200);
...
static void cp2130_read_otprom(struct work_struct *work)
{
	...
	for (i = 0; i < CP2130_NUM_GPIOS; ++i) {
		dev_info(&dev->udev->dev, "pin %d: %02X", i, urb[i]);
...
static void cp2130_read_gpios(struct work_struct *work)
{
	...
	mutex_lock(&dev->usb_bus_lock);
	i = usb_control_msg(dev->udev, recv_pipe,
 ...
 out:
	schedule_delayed_work(&dev->irq_work,
			      usecs_to_jiffies(dev->irq_poll_interval));
}
...
static int cp2130_gpio_irq_probe(struct cp2130_device *dev)
{
	...
	irq_dev->irq_domain =
	    irq_domain_add_linear(dev->udev->dev.of_node, CP2130_NUM_GPIOS,
	...
	irq_dev->irq_mask = 0;
	for (i = 0; i < CP2130_NUM_GPIOS; ++i) {
		irq_dev->virq[i] = irq_create_mapping(irq_dev->irq_domain, i);
		dev_dbg(&dev->udev->dev, "created virtual irq %d",
			irq_dev->virq[i]);
	}

	INIT_DELAYED_WORK(&dev->irq_work, cp2130_read_gpios);
	schedule_delayed_work(&dev->irq_work,
			      usecs_to_jiffies(dev->irq_poll_interval));
...
int cp2130_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	/* create sysfs files */
	ret = device_create_file(&intf->dev, &dev_attr_channel_config);
	if (ret)
		dev_err(&udev->dev,
			"failed to create channel_config sysfs entry");
	ret = device_create_file(&intf->dev, &dev_attr_otp_rom);
	if (ret)
		dev_err(&udev->dev, "failed to create otp_rom sysfs entry");
	ret = device_create_file(&intf->dev, &dev_attr_irq_poll_interval);
	if (ret)
		dev_err(&udev->dev,
...
void cp2130_disconnect(struct usb_interface *intf)
{
	...
	cancel_delayed_work_sync(&dev->irq_work);

	cp2130_gpio_irq_remove(dev);

	/* remove sysfs files */
	device_remove_file(&intf->dev, &dev_attr_channel_config);
	device_remove_file(&intf->dev, &dev_attr_otp_rom);
